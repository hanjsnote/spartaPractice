...
public class JdbcTemplateMemoRepository implements MemoRepository{
    ...
    public MemoResponseDto saveMemo(Memo memo) {
        //jdbcTemplate를 사용하면 Query를 직접 작성하지 않아도 된다. SimpleJdbcInsert가 자동으로 insert쿼리를 만들어준다.
        SimpleJdbcInsert jdbcInsert = new SimpleJdbcInsert(jdbcTemplate);  //insert 작업을 준비
        jdbcInsert.withTableName("memo").usingGeneratedKeyColumns("id"); //memo 테이블에 저장 id는 자동 생성되는 pk컬럼이 무엇인지 알려줌

        Map<String, Object> parameters = new HashMap<>();
        parameters.put("title", memo.getTitle());
        parameters.put("contents", memo.getContents());

        Number key = jdbcInsert.executeAndReturnKey(new MapSqlParameterSource(parameters)); //insert 실행이며 insert된 row의 자동 생성된 id값을 반환
